...
def ClassifyChunk(F,Flist,ChunkClusters,TP,start):
	...
	return (chunkClassification.replace("-sample.chunk",""),secChoice)

def driver(start):
	if os.path.isfile("ChunkClusters.pkl"):
		os.remove("ChunkClusters.pkl")
	train = ['engineering-sample.chunk', 'health-sample.chunk','culture-arts-entertainment-sample.chunk','education-science-sample.chunk', 'politics-society-sample.chunk', 'computers-sample.chunk', 'sports-sample.chunk','business-sample.chunk']
	...
	print("Classification for the chunk : ",result)
